...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  // Log the request
  console.log(`--- ADMIN BOOKING UPDATE [PUT] HIT for ID: ${params.id} ---`);
  ...
  try {
    ...
    console.log(`Attempting to update booking ${params.id} with:`, updateObj);
    ...
    console.log(`Successfully updated booking ${params.id}`);
    return NextResponse.json({ success: true, message: "Booking updated successfully" });

  } catch (error) {
    console.error(`!!! ADMIN BOOKING UPDATE [PUT] ERROR for ID: ${params.id}:`, error);
